...
Status: implementation plan, grounded in the current repository on 2026-09-15.
...
| Receiver scaffold | Repository root `webhook/server.js` | Reuse authentication, body-size limits and sanitized responses for a synthetic connectivity test. It currently discards the request body after acknowledging it. |
...
4. **The receiver does not retain events.** Its `200` response causes the phone to delete the queued content. Use it only for synthetic transport checks until durable ingestion exists.
...
Use the repository Gradle wrapper with a compatible JDK and Android SDK. The project compiles against SDK 36 and targets JVM 17. The previous implementation report records a missing-Java blocker; resolve the toolchain before device testing.
...
The hostname and private-network access mechanism remain deployment decisions. If a private network such as Tailscale is chosen, verify connectivity from the phone on mobile data as well as home Wi-Fi. Preserve HTTPS and bearer authentication.
...
The current receiver cannot demonstrate persisted payload contents. Add a test-only assertion against the expected synthetic JSON, or use the durable receiver from the next milestone to inspect a synthetic record. Do not turn raw banking-payload logging on to debug this step.
...
This document defines the plan. It does not deploy a receiver, change Android behavior or establish device validation.
